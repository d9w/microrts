 ...
 * @author santi
 */
public class ExampleRush extends AbstractionLayerAI {
    ...
    // If we have any "light/heavy/ranged": send it to attack to the nearest enemy unit
    // If we have a base: train worker until we have 1 workers
    // If we have a barracks: train a unit (randomly choosing between light, ranged or heavy)
    // If we have a worker: do this if needed: build base, build barracks, harvest resources
    public ExampleRush(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }

    public ExampleRush(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        utt = a_utt;
    ...
    public void barracksBehavior(Unit u, Player p, PhysicalGameState pgs) {
        ...
        if (p.getResources() >= lightType.cost) {
            candidates.add(lightType);
        }
        if (p.getResources() >= heavyType.cost) {
            candidates.add(heavyType);
        }
        if (p.getResources() >= rangedType.cost) {
            candidates.add(rangedType);
        }

        if (!candidates.isEmpty()) {
            train(u, candidates.get(r.nextInt(candidates.size())));
        }
